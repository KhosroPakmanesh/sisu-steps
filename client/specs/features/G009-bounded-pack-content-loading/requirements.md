...
- **REQ-G009-009:** Full-backup restoration shall validate the current backup contract and referenced content against all installed full packs before replacing current data. The original exact installed pack/version-set clause is superseded by root `REQ-G008-020`–`024`: new packs initialize empty, and changed installed packs follow the disclosed compatibility-loss policy.
...
- **REQ-G009-013:** The current learner-state and backup schema numbers and serialized fields shall remain unchanged. Stored state with an obsolete shape or a version entry for a removed pack shall reset completely; an obsolete backup or one containing a removed pack shall be rejected atomically. Root `REQ-G008-020`–`024` supersede the former exact pack/version-set rejection rule for otherwise valid backups. No compatibility migration, fallback reader, legacy alias, or transitional format shall be introduced.
