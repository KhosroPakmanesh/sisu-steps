...
- Use `--surface-stationery` for neutral warm-paper objects such as assignments, the bound topic-catalog sheet, lesson/reference sheets, common-mistake advisories, and ledgers. Common-mistake reference sheets retain an amber edge and explicit caution wording without becoming active warning surfaces. Reserve `--surface-warning` for active warning, incorrect-answer, and review meaning; visual paper texture does not make a neutral object a warning.
- Use `--surface-review` for the shared blue/amber review-paper tint on cumulative Review cards and selected lesson-navigation cards. The tint communicates a current review-oriented selection; `aria-current`, explicit labels, movement, and depth keep that state understandable without colour alone.
...
- Preserve available-width lesson reading; do not introduce a maximum prose-width cap. Preserve the approved outer folder geometry and established feature spacing at default text size from 48rem upward. Below 48rem, remove folder and clip geometry from layout, use safe-area-aware outer paper insets and compact content padding that clears the left ledger/binding strip, keep at least the viewport width minus 40 pixels available to routed content, and reduce decorative vertical travel with existing spacing tokens rather than smaller type or controls. Sticky phone navigation retains no enclosing surface of its own; suitable four-value summaries reuse the existing two-column grid above 20rem of available workbook width, and test status marks reuse their larger-layout heading-row alignment. Study actions use existing surface, border, shadow, touch-target, and safe-area values. At 20rem or less of available shell width, enlarged text may use physical `--space-reflow-inline` padding and stacked summaries, badges, and answer controls; the Study dock returns to ordinary flow instead of consuming the enlarged reading viewport. Required typography and practical control heights remain relative to the requested text size.
