...
- Preserve available-width lesson reading; do not introduce a maximum prose-width cap. Preserve the approved outer folder geometry at default text size from 48rem upward. Below 48rem, remove folder and clip geometry from layout, use safe-area-aware outer paper insets and compact equal paper padding, and keep at least the viewport width minus 40 pixels available to routed content at the default text scale. Below 20rem of available shell width, enlarged text may use physical `--space-reflow-inline` padding and stacked answer controls. Required typography and practical control heights remain relative to the requested text size.
