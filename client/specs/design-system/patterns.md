...
- Topic progress and both Stats hero summaries reuse one `.assignment-summary` card layout, equal padding on all four sides, equal metric-cell padding, compact content-sized rows and shared gap, type scale, warm taped material, and subtle horizontal hover shuffle. In wide two-column sheets, use content-sized metric columns and distribute the spare width across both side gutters and the middle gap, keeping each label and value left-aligned within its group. Keep narrow two-column sheets flexible and stack metrics when workbook space is too narrow. In wide heroes, place the return link above the introduction and center-align the self-sized sheet beside it without stretching the sheet to the copy's height or adding minimum row heights; name the topic Stats sheet accessibly without adding a detached visible heading above it.
