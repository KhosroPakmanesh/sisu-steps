...
export async function validatePackContent(pack) {
  ...
  const checkFocus = (item, label) => {
    ...
    if (!hasTextArray(item.prerequisiteSkills))
      errors.push(`${label}: invalid prerequisite skills`);
  ...
  for (const [testIndex, test] of pack.tests.entries()) {
    ...
    if (
      ...
      referencedLessons.some(
        (lesson) =>
          lesson.stage !== 'focused' || lesson.targetSkills?.[0] !== test.targetSkills?.[0],
      )
    )
    ...
    if (test.stage === 'review')
      for (const skill of declaredSkills)
        if (!focusedCoveredSkills.has(skill))
          errors.push(
            `${test.id}: review introduces a skill not covered by focused tests: ${skill}`,
          );
    ...
    for (const exercise of test.exercises ?? []) {
      ...
      if (
        test.stage === 'focused' &&
        !(exercise.requiredSkills ?? []).includes(test.targetSkills[0])
      )
  ...
  if (typeof pack.id !== 'string' || !/^[a-z0-9][a-z0-9-]*$/u.test(pack.id)) {
    ...
    const validatorFamilies = new Map([
      ['vowel-harmony-location-endings', 'foundations'],
      ['kpt-singular-forms', 'foundations'],
      ['t-plural-agreement', 'foundations'],
  ...
  const lessonVocabularyCounts = {
    ...
    supplied: lessons.reduce(
      (total, lesson) => total + (lesson.suppliedVocabulary?.length ?? 0),
      0,
    ),
  };
  return {
