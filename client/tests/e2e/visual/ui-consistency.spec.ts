...
for (const theme of ['Day', 'Night']) {
  test.describe(`${theme} UI consistency`, () => {
      ...
      const owners = await page.evaluate(() => {
        ...
        return {
          ...
          sequenceMarkerVisual: rules
            .filter(
              (rule) =>
                ['.sequence-marker', '.test-number', '.archive-number', '.stats-name > span'].some(
    ...
    test('keeps topic results and their clear actions on one punched ledger', async ({ page }) => {
      ...
      const statsLedger = page.locator('.stats-ledger.ledger-sheet').first();
      const expected = await statsLedger.evaluate(materialRecipe);
    ...
    test('keeps repeated semantic colour roles consistent on every feature page', async ({
      ...
      const routes: [string, string][] = [
        ...
        [TOPIC_STATS, '.stats-overview, .ledger-sheet'],
      ];
    ...
    test('uses one visual recipe for sequence markers across feature pages', async ({ page }) => {
      ...
      await open(page, TOPIC_STATS);
      expect(
        await page.locator('.stats-name > .sequence-marker').first().evaluate(markerRecipe),
      ).toEqual(expected);
