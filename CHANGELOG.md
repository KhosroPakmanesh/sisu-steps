...
- Optional **Learn first** preparation for all fourteen tests with reusable beginner lessons, worked examples, common mistakes, separate unscored practice, and local completion tracking.
- Visible **Focused**, **Guided combination**, and **Review** stages with declared target skills, prerequisites, and lesson vocabulary.
...
- Core and extended test sets, with a declared important-skill coverage contract and visibly separate dashboard groups.
...
- The grammar-foundation content pack is now version 4.0.0. Focused tests avoid hidden stem changes and irregular verb construction; compound grammar is explicitly identified as guided combination or review.
...
- Grammar packs may now contain 200–1,000 authored scored exercises according to coverage needs instead of being fixed at exactly 200; core tests must cover every declared important skill before extended tests begin.
...
- Full core/extended test maps and objectives now live on their owning topic page instead of expanding every installed pack on the landing page; lesson and study exits return to that topic.
