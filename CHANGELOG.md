...
- Added complete target English meanings before all 21 Finnish sentence construction and completion prompts that previously omitted them across plural sentences, KPT verbs, and the cumulative Review; Finnish-to-English tasks remain unchanged, the plural-sentences lesson advances to version 6.1.0, and universal validation now prevents the omission from recurring.
- Reworked the KPT-verb lesson, Focused test and cumulative-review questions around short, complete A1 `Minä + verb + context` sentences, with supplied context words, full sentence breakdowns, five optional practice questions, corrected beginner verb selection, natural present/future translation alternatives, response-format-aligned mastery pairs and feedback that accurately distinguishes shown stems from explained stems; pack and lesson version 6.1.0 protects earlier progress from materially changed prompts and answers.
- Made the Vitest and Playwright harnesses deterministic on Node 26 and Windows by installing isolated test storage, serving the production build within Playwright's own lifecycle, and matching hashed production stylesheet assets when enlarged-text checks inject the requested root size.
