...
- **VAL-G001-002** (`REQ-G001-001`, `003`–`005`): Validate the bundled pack schema, unique stable IDs, stored `0 - A1.3` level ranges, 200–1,000 scored exercises, exercise types, and topic coverage. Browser checks verify that catalog, topic, reports, and Data & backup surfaces prepend the visible `Level:` label.
...
- **VAL-G001-074** (`REQ-G001-086`–`095`, `112`): Catalog, topic, lesson, study, reports, Data & backup, persistence, backup, and browser checks verify that all four installed packs are independently reachable and isolated, the catalog total remains 720, existing vowel-harmony progress remains compatible, and every workflow remains local-only and usable at 320-, 768-, and 1440-pixel widths.
