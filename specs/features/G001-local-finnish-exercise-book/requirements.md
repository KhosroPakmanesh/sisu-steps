...
- **REQ-G001-119:** Existing `foundations-review` exercises shall be partitioned by their actual target skill across the successor Reviews without duplication. On first loading the successor catalog, learner attempts, unfinished sessions, mistakes, corrections, mastery, lesson completions, and lesson notes from the retired pack shall be remapped to the new owner. Completed and unfinished `foundations-review` records shall be split by successor ownership. The former broad topic note shall move unchanged to `kpt-singular-forms`. No compatible learner record shall be silently cleared.
