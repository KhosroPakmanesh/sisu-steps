...
- Content-catalog service tests reject invalid or duplicate catalog group declarations before either Stats or Notebook receives summaries, keeping both catalogs on the same validated metadata and authored order.
- `npx.cmd playwright test tests/e2e/content/pack-groups.spec.ts tests/e2e/workflows/content-loading.spec.ts --config=playwright.review.config.ts`: 27 checks passed across the configured mobile, tablet, and wide Chromium projects, including matching labeled groups and pack order on `/` and `/stats`, catalog startup, retry, and lazy full-pack loading. The disposable port-4211 review server and config were removed after the run because an unrelated process occupied the standard port 4200.
- No progress calculation, learner-data operation, persistence contract, or clearing consequence changed.
